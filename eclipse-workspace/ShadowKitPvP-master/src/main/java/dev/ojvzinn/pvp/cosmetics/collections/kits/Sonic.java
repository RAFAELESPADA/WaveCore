...
public class Sonic extends Kit {
    ...
    public Sonic(String key, YamlConfiguration CONFIG) {
        super(key, CONFIG);
    }
    ...
    public void applyKit(Player player) {
        ...
        inventory.setItem(1, BukkitUtils.deserializeItemStack("406 : 1 : nome>&6Sonic &7(Clique direito)"));
        ...
        player.sendMessage("§aVocê equipou o kit Sonic.");
    }
    ...
    public void onPlayerInteractAbility(PlayerInteractEvent event) {
        ...
        if (KitPvP.isPlayingKitPvP(player)) {
            ...
            if (item != null && item.getType() == Material.QUARTZ && item.getItemMeta() != null && item.getItemMeta().hasDisplayName() && item.getItemMeta().getDisplayName().equals("§6Sonic §7(Clique direito)") && isSelected(profile)) {
                ...
                for (Entity nearEntity : profile.getPlayer().getNearbyEntities(3, 0, 3)) {
                    if (nearEntity instanceof Player) {
                        ((Player) nearEntity).addPotionEffect(new PotionEffect(PotionEffectType.POISON, 20 * 5, 1), true);
                        nearEntity.sendMessage("§a[Sonic] §fVocê foi envenenado pelo jogador " + player.getName() + " por 5 segundos.");
                    }
                }

                player.setVelocity(player.getLocation().getDirection().normalize().multiply(4).setY(0));
                player.sendMessage("§a[Sonic] §fVocê envenenou " + profile.getPlayer().getNearbyEntities(3, 0, 3).size() + " inimigos!");
