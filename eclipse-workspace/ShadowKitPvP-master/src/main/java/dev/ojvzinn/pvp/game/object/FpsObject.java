...
public class FpsObject extends KitPvP {

    private final Map<String, Kit> oldKits = new HashMap<>();

    public FpsObject(String configName, WarpEnum warp) {
    ...
    public void join(Profile profile) {
        ...
        if (spawn == null) {
            profile.getPlayer().sendMessage("§cA arena FPS ainda não foi configurada!");
            return;
        }
        ...
        oldKits.put(player.getName(), Cosmetic.findCosmeticSelected(CosmeticType.KIT, profile));
        Cosmetic.findById("1", CosmeticType.KIT).selectedCosmetic(profile);
    ...
    public void leave(Profile profile) {
        ...
        oldKits.get(profile.getName()).selectedCosmetic(profile);
    ...
    public KitPvPConfig setupConfig(String configName) throws IOException {
        FpsConfig config = new FpsConfig(configName);
        Main.getInstance().getLogger().info(config.getWorldname());
        return config;
    }

    public FpsConfig getConfig() {
        return getConfig(FpsConfig.class);
    }
}
